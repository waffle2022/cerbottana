...
@command_wrapper(
    aliases=("gtsevent", "gtsranked"),
    helpstr="guessthesprite ma con un tempo limite e una leaderboard separata!",
    allow_pm=False,
...
class GuessTheSprite:
    ...
    async def cmd_func(cls, msg: Message) -> None:
        ...
        while msg.room in cls.active_games:
            if msg.room in cls.active_games:
                if duration < time.monotonic - game.start:
    ...
    async def on_message(cls, msg: RawMessage) -> None:
        ...
        score[msg.user.username] = score.get(msg.user.username, 0) + player.gts_points
